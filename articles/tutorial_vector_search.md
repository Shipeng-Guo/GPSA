...
While Up/Down queries use only the most extreme genes, **vector search**
uses the entire gene expression profile as a query. This approach:
...
1.  **Start with default parameters**: `eta = 0.90`, `min_k = 300`,
    `max_k = 5000`
2.  **Compare with Up/Down**: Vector search should give similar but
    potentially better results
3.  **Check compile info**: Understand how many genes are actually used
4.  **Use higher eta for subtle signatures**: If changes are small,
    retain more genes
5.  **Validate top hits**: Always examine GSEA plots for biological
    relevance
